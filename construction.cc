...
G4VPhysicalVolume* MyDetectorConstruction::Construct()
{
        ...
        G4Box *sTarget = new G4Box("sTarget", 10*cm, 10*cm, 1*mm);  // tinta subtire pentru fluenta
        G4LogicalVolume *lTarget = new G4LogicalVolume(sTarget, air, "lTarget");
        G4VPhysicalVolume *pTarget = new G4PVPlacement(0, G4ThreeVector(0, 0, 5*cm), lTarget, "pTarget", logicWorld, false, 0);
